...
function Validate(input){
    let errors={}
    if (/[`!@#$%^&*()_+\-=[\]{};':"\\|,.<>\d/?~]/.test(input.name)){ errors.name = 'name can not contain numbers or special characters'
}
   ...
   if(/^(?=.*[A-Z].*[A-Z])(?=.*[!@#$&*])(?=.*[0-9].*[0-9])(?=.*[a-z].*[a-z].*[a-z]).{8}$/.test(input.name)){
        ...
        <div>
            <form onSubmit={(el)=>handleSubmit(el)} >
                <div>
                    ...
                    <input type="text" value={input.name} name={"name"} onChange={(el)=>handleChange(el)}/>
                    <br />
                    {errors.name ? <label>{errors.name}</label> : null}
                </div>
                <div>
                    <label htmlFor="">Email:</label>
                    <input type="text" value={input.email} name={"email"} onChange={(el)=>handleChange(el)}/>
                    <br />
                    {errors.email ? <label>{errors.email}</label> : null}
                </div>
                <div>
                    <label htmlFor="">Password:</label>
                    <input type="password" value={input.password} name={"password"} onChange={(el)=>handleChange(el)}/>
                    <br />
